...
import { promiseHandle, log, formatNumber } from '../utils_schedele/utils_schedele';
...
Page({
  ...
  listItemLongTapEvent(e) {
    ...
    //如果不是编辑勾选模式下才生效
    if (!isEditMode) {
      wx.navigateTo({ url: '../scheduleDetail/detail?id=' + id });
    }
  },
  // listItemLongTapEvent(e) {
  //   const {isEditMode} = this.data;
  ...
  todoInputChangeEvent(e) {
    var that = this;
    var {value} = e.detail;
    that.setData({ todoInputValue: value });
  },
  ...
  todoTextAreaChangeEvent(e) {
    var that = this;
    var {value} = e.detail;
    that.setData({ todoTextAreaValue: value });
  },
  ...
  saveDataEvent() {
    ...
    if (todoInputValue !== '' && levelSelectedValue != null) {
      ...
      showModal.call(this, '请填写事项标题');
    } else {
      showModal.call(this, '请选择事项等级');
    }
  },
...
function loadItemListData() {
  const {year, month, date} = this.data.data.selected;
  let _this = this;
  var value = wx.getStorageSync(Config.ITEMS_SAVE_KEY);
  var arrayt=[];
  for (var i = 0, l = value.length; i < l; i++) {
    if (value[i].year == year && value[i].month == month-1 && value[i].date == date ){
      arrayt.push(value[i]);
    }
  }
  _this.setData({ itemList: arrayt });
